...
function ProductForm({ productId, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (productId) await axios.put(`${API}/ha/products/${productId}`, body);
      else await axios.post(`${API}/ha/products`, body);
      onSaved();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-2xl max-w-xl w-full p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button onClick={save} data-testid="ha-pf-save" className="px-3 py-1.5 text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white rounded">Save</button>
        </div>
      </div>
    </div>
  );
