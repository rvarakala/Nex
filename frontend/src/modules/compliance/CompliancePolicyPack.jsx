...
import {
  ...
  CheckCircle2, AlertTriangle, Circle, PenLine, X, Loader2, Menu, ChevronDown,
} from 'lucide-react';
...
export default function CompliancePolicyPack() {
  ...
  const loadPolicy = async (pid) => {
    ...
    setDrawerOpen(false); // auto-close mobile drawer on selection
    try {
  ...
  // Active policy meta — for the mobile top-bar pill.
  const activePolicyMeta = policies.find((p) => p.id === active);

  return (
    <div className="p-4 md:p-6 space-y-4 bg-slate-50 min-h-screen" data-testid="compliance-page">
      ...
      {/* Mobile pill — opens the policy list drawer. Hidden on md+. */}
      {policies.length > 0 && (
        <button
          onClick={() => setDrawerOpen(true)}
          data-testid="compliance-mobile-menu-toggle"
          className="md:hidden w-full flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-lg shadow-sm text-sm font-semibold text-slate-700 hover:bg-slate-50 active:bg-slate-100"
        >
          <Menu size={14} />
          <span className="truncate flex-1 text-left">
            {activePolicyMeta ? activePolicyMeta.title : 'Pick a policy'}
          </span>
          <span className="text-[10px] font-mono text-slate-400">{policiesSigned}/{policiesTotal}</span>
      ...
      {/* Mobile drawer */}
      {drawerOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex" data-testid="compliance-mobile-drawer">
          <button
            aria-label="Close policy list"
            onClick={() => setDrawerOpen(false)}
            className="absolute inset-0 bg-slate-900/50"
          />
          <div className="relative w-80 max-w-[85vw] bg-white h-full shadow-xl flex flex-col animate-in slide-in-from-left duration-150">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 shrink-0">
              <div className="flex items-center gap-2">
                <ShieldCheck size={16} className="text-emerald-600" />
                <div className="text-[11px] uppercase tracking-wider font-bold text-slate-500">Policies</div>
              </div>
              <button
                onClick={() => setDrawerOpen(false)}
                data-testid="compliance-mobile-drawer-close"
            ...
            <div className="flex-1 overflow-y-auto p-3">
              <PolicyList
                policies={policies}
                adoptionFor={adoptionFor}
                loadPolicy={loadPolicy}
                active={active}
                policiesTotal={policiesTotal}
                policiesSigned={policiesSigned}
              />
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-4">
        {/* Desktop policy list — hidden on < md */}
        <aside className="hidden md:block bg-white border border-slate-200 rounded-2xl p-3 shadow-sm h-fit sticky top-4" data-testid="compliance-list">
          <PolicyList
            policies={policies}
            adoptionFor={adoptionFor}
            loadPolicy={loadPolicy}
        ...
        {/* Reader pane */}
        <main className="bg-white border border-slate-200 rounded-2xl p-4 md:p-6 shadow-sm min-w-0" data-testid="compliance-reader">
...
function PolicyList({ policies, adoptionFor, loadPolicy, active, policiesTotal, policiesSigned }) {
  return (
    <>
