...
export default function NewPatientPage() {
  ...
  const submit = async (action) => {
    ...
    try {
      ...
      setErr(typeof d === 'string' ? d : (e?.message || 'Registration failed'));
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="p-4 max-w-6xl mx-auto" data-testid="new-patient-page">
      <div className="bg-white rounded-lg shadow-sm border border-slate-200">
        ...
        <div className="p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
                {dupMatches.slice(0, 4).map((m) => (
                  <button
                    key={m.patient_id}
                    type="button"
                    onClick={() => navigate(`/frontdesk/returning?q=${encodeURIComponent(m.mobile || m.name || m.mrd || '')}`)}
          ...
          {err && <div className="mt-3 text-xs text-red-700 bg-red-50 border border-red-200 rounded px-3 py-2" data-testid="form-error">{err}</div>}
        </div>
