...
- [Feb 2026] **Speech Audiometry section added to the printed report**. New `reports/sections/SpeechSection.js` renders the 4 stacked blocks in order: (1) Speech Audiometry table, (2) Speech Audiogram (WRS curves, live canvas), (3) Word Recognition table (with Word List / Presentation header), (4) Word Recognition in Noise table. Each sub-block auto-hides if empty. Controlled by the existing "Speech Audiometry" toggle in the Report Builder sidebar (default OFF).
