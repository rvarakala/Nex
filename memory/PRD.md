...
- [Feb 2026] **Speech audiogram behaves like Pure Tone**: symbols now match clinical convention — O / △ for Right unmasked/masked, X / □ for Left unmasked/masked, S for Soundfield, A for Soundfield Aided (replacing generic colored circles). Left-click plots/replaces a point at the cursor snap position. **Right-click context menu** with three actions: "Plot No Response @ X dB, Y%", "Delete point @ X dB", "Clear active channel". Legend in the left control panel updated to match. Report's `SpeechSection` print layout reworked from flex-wrap to CSS grid — WRN row converted to a table so nothing overflows the A4 page (verified: `maxScrollW=703 < clientWidth=794`).
- [Feb 2026] **Results section now dynamic based on enabled test sections**. Added a third narrative — **Speech Audiometry Findings** (new backend field `speech_findings` + sidebar textarea). `ResultsGridSection` rewritten to accept an `entries` array and auto-layout 1 / 2 / 3 cells at full-width / 2-col / 3-col. Only shows findings whose parent test section is currently toggled ON in the Report Builder — e.g. enabling only Speech shows a single full-width Speech Findings cell; enabling PT + Speech shows 2 cells; all three enabled shows the full 3-col row.
- [Feb 2026] **All 6 P2 clinical tabs built with matching report sections** (replaces placeholders):
  1. **Special Tests** — SISI, TDT (500/2k), ABLB, MLB, Stenger, Bekesy (Type I-V), site-of-lesion impression.
  2. **Otoacoustic Emissions** — DPOAE (7-freq f₂ table with SNR/Response/Noise-floor × R/L), TEOAE (5-freq SNR + reproducibility), Pass/Refer, stimulus, impression.
  3. **Sound Field / Aided** — Setup (stimulus, azimuth, distance, calibration), Unaided + Aided warble-tone thresholds, Speech scores (SRT/WRS aided vs unaided), functional gain impression.
  4. **ABR / ASSR** — Stimulus params, Wave I/III/V latencies + inter-peak I-III / III-V / I-V + morphology per ear, estimated thresholds, 4-freq ASSR table, neural integrity impression.
  5. **Pediatric** — Chronological/developmental age, Technique (BOA/VRA/Play), Reliability, AC+BC behavioural thresholds (4 freqs), Soundfield, Ling 6 + SAT, Parent reports (IT-MAIS, P.E.A.C.H.), impression.
  6. **Tinnitus** — History (duration/onset/character/triggers), Psychoacoustic per ear (pitch match, loudness match, MML, residual inhibition), THI score/grade + F/E/C subscales, management plan, impression.
...
- [Feb 2026] **Graph visualisation for ABR + Sound Field**:
